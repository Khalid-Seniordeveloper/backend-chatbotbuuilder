...
async function injectWidget() {
  ...
  //* Get the chatbot color and add the color in chatbot config obj
  const chatbotColor = script.getAttribute("data-color");
  //* Check if the provided color is a valid hex string
  const hexColorRegex = /^#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/;
  if (chatbotColor) {
    if (hexColorRegex.test(chatbotColor)) {
      chatbotConfig.color = chatbotColor;
    } else {
      throw Error(`Color must be a valid hex-color value.`);
    }
  } else {
    chatbotConfig.color = "#007bff";
  }
