...
const createChatWidget = (chatbotConfig) => {
  ...
  chatWidget.innerHTML = `<div id="chat-box-container" class="hidden">
        ...
        <div id="chat-header">
          ...
          <div class="chat-header-controls">
            ...
            <i class="ri-arrow-down-double-line icon control-icon"></i>
          </div>
        </div>
  ...
  //* Get the button using button's id and add event listener to show/hide animation on hovering the button
  const toggleWidgetButton = chatWidget.querySelector("#widget-button");

  toggleWidgetButton.addEventListener("mouseover", () =>
    showHoverEffect(
      toggleWidgetButton,
      chatWidget,
  ...
  toggleWidgetButton.addEventListener("mouseleave", () =>
    hideHoverEffect(toggleWidgetButton)
  );

  //* Button to close chatbox
  const closeChatboxButton = chatWidget.querySelector(
    ".ri-arrow-down-double-line"
  );
  closeChatboxButton.addEventListener("click", () =>
    closeChatbox(chatWidget, chatbotConfig.greetingMessage)
  );
...
const getChatbotData = async (chatbotConfig) => {
  const url = `http://localhost:5000/api/chatbot/single/${chatbotConfig.id}`;
...
const sendMessage = async (container, input, chatbotConfig) => {
  ...
  if (question.message) {
    ...
    const url = `http://localhost:5000/api/chatbot/ask?question=${encodeURIComponent(
      question.message
    )}&chatbotId=${id}`;
    ...
    try {
        ...
        for (const line of lines) {
          if (line.startsWith("data: ")) {
            ...
            if (data === "[DONE]") {
              ...
              text.innerHTML = markdownToHtml(botResponse);
              answer.message = botResponse;
              saveMsgToStorage(id, answer);
              console.log("Original Bot Response ===>>>", botResponse);
              console.log(
                "Formatted Markdown ===>>>",
                markdownToHtml(botResponse)
              );
            } else {
              botResponse += data + " ";
              text.innerHTML = markdownToHtml(botResponse);
      ...
      text.innerHTML = markdownToHtml(answer.message);
      const time = document.createElement("p");
      time.className = "message-time";
      answer.time = new Date(Date.now()).toLocaleTimeString("en-US", {
        hour: "2-digit",
...
const renderMessage = (container, message, img) => {
  ...
  const text = document.createElement("div");
  text.innerHTML = markdownToHtml(message.message);
...
//? Function to format markdown language in html
function markdownToHtml(markdown) {
  ...
  // This pattern matches an asterisk-delimited text not at the very beginning of a line.
  html = html.replace(
    /([^\n^*]|^[^*\n])\*([^*\n]+?)\*(?!\*)/g,
    "$1<em>$2</em>"
  );

  // Process lists: split into lines to handle list items
  const lines = html.split("\n");
  let inList = false;

  for (let i = 0; i < lines.length; i++) {
    if (lines[i].match(/^\s*\*\s+/)) {
      // If this is the first list item, prepend <ul>
      if (!inList) {
        lines[i] = "<ul>" + lines[i].replace(/^\s*\*\s+(.*)$/, "<li>$1</li>");
        inList = true;
      } else {
        lines[i] = lines[i].replace(/^\s*\*\s+(.*)$/, "<li>$1</li>");
      }

      // If the next line is not a list item, append </ul>
  ...
  // Remove <br> tags before or after list elements so they don't break the HTML structure
  html = html.replace(/<br>(?=<\/?(ul|li)>)/g, "");
...
const openChatBox = (chatWidget) => {
  ...
  if (chatbox.classList.contains("hidden")) {
    ...
    chatWidget.removeChild(toggleWidgetButton);
  }
};

//? Function to close the chatbox
const closeChatbox = (chatWidget, greetingMessage) => {
  const chatbox = chatWidget.querySelector("#chat-box-container");
  const toggleWidgetButton = document.createElement("button");
  toggleWidgetButton.id = "widget-button";

  let chatIcon = document.createElement("i");
  chatIcon.classList.add("ri-chat-ai-fill");
  chatIcon.id = "widget-button-icon";

  if (!chatbox.classList.contains("hidden")) {
    chatbox.classList.add("hidden");
    toggleWidgetButton.append(chatIcon);
    chatWidget.append(toggleWidgetButton);
    if (chatWidget.contains(chatIcon) && chatIcon.offsetParent !== null) {
      chatIcon.style.transition = "transform 0.4s ease";
      chatIcon.style.transform = "rotate(360deg)";
    }

    toggleWidgetButton.addEventListener("mouseover", () =>
      showHoverEffect(toggleWidgetButton, chatWidget, greetingMessage)
    );
...
//? Function to show animation on hovering the widget button
const showHoverEffect = (toggleWidgetButton, chatWidget, greetingMessage) => {
  if (!toggleWidgetButton.classList.contains("hovered")) {
    console.log("Mouseover function called");
    toggleWidgetButton.classList.add("hovered");

    //* Get the original chat icon element
    let chatIcon = toggleWidgetButton.querySelector(".ri-chat-ai-fill");

    //* Create the greeting message element, initially transparent
    let greetingMessageElem = document.createElement("p");
    greetingMessageElem.id = "hover-greeting-message";
    greetingMessageElem.innerText = greetingMessage;
    greetingMessageElem.style.opacity = "0";
    greetingMessageElem.style.transition = "opacity 0.2s ease";

    //* Create the expand icon element
    let expandChatboxIcon = document.createElement("i");
    expandChatboxIcon.classList.add("ri-arrow-up-double-fill");
    expandChatboxIcon.id = "widget-button-icon";

    //* Set a transition on the chat icon for a smooth rotation
    if (chatIcon) {
      ...
      chatIcon.addEventListener(
        "transitionend",
        () => {
          //* Remove the original chat icon once the rotation is complete
          if (toggleWidgetButton.contains(chatIcon)) {
            toggleWidgetButton.removeChild(chatIcon);
          }
          //* Append the greeting message if not already appended
          if (!toggleWidgetButton.querySelector("#hover-greeting-message")) {
            toggleWidgetButton.append(greetingMessageElem);
          }
          //* Append the expand icon if not already appended
          if (!toggleWidgetButton.querySelector(".ri-arrow-up-double-fill")) {
            toggleWidgetButton.append(expandChatboxIcon);
          }
          //* Fade in the greeting message immediately
          greetingMessageElem.style.opacity = "1";
        },
        { once: true }
      );
    }

    //* Attach event listener to the expand icon to toggle widget visibility
    expandChatboxIcon.addEventListener("click", () => openChatBox(chatWidget));
  }
};

//? Function to show animation on hovering away from the widget button
const hideHoverEffect = (toggleWidgetButton) => {
  if (toggleWidgetButton.classList.contains("hovered")) {
    console.log("Mouseleave function called");
    toggleWidgetButton.classList.remove("hovered");

    //* Remove all elements with ID "hover-greeting-message"
    let greetingMessageElems = toggleWidgetButton.querySelectorAll(
      "#hover-greeting-message"
    );
    greetingMessageElems.forEach((elem) => {
      toggleWidgetButton.removeChild(elem);
    });

    //* Remove all elements with the class "ri-arrow-up-double-fill"
    let expandChatboxIcons = toggleWidgetButton.querySelectorAll(
      ".ri-arrow-up-double-fill"
    );
    expandChatboxIcons.forEach((elem) => {
      toggleWidgetButton.removeChild(elem);
    });

    //* Append the new chat icon only if one doesn't exist
    if (!toggleWidgetButton.querySelector(".ri-chat-ai-fill")) {
      let chatIcon = document.createElement("i");
      chatIcon.classList.add("ri-chat-ai-fill");
      chatIcon.id = "widget-button-icon";
      toggleWidgetButton.append(chatIcon);
