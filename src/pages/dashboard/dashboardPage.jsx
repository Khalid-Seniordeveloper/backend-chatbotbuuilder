...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      if (editModalOpen || isModalOpen || selectedBotForEmbed) {
        document.body.classList.add("active-modal");
      } else {
        document.body.classList.remove("active-modal");
      }
    }
  }, [editModalOpen, isModalOpen, selectedBotForEmbed]);
  ...
  const handleBotClick = (botId) => {
    router.prefetch(`/dashboard/chatbot/${botId}`);
  ...
  const handlePreviewWidget = (botData) => {
    ...
    router.push(
      `/widget?name=${botData.name}&tagline=${botData.tagline}&id=${botData._id}&size=${config.widgetSize}&movable=${config.isMovable}&color=${encodedColor}`
    );
  };
  ...
  const deleteChatbot = async (botId) => {
    try {
      const response = await axios.delete(
        "http://localhost:5000/api/chatbot/delete",
  ...
  const handleStatusToggle = async (chatBotId) => {
    try {
      const response = await axios.post(
        "http://localhost:5000/api/chatbot/toggleChatbotStatus",
        {
          chatBotId,
        }
      );
  ...
  const editChatbot = async (botId) => {
    ...
    try {
      const response = await axios.put(
        ` http://localhost:5000/api/chatbot/edit/${botId}`,
        {
  ...
  return (
    <>
      ...
      <div className={styles.tableContainer}>
        ...
        <div className="w-full bg-[#fff] rounded-2xl h-auto p-4">
          <table className="w-max sm:w-max md:w-max lg:w-full rounded-lg bg-[#fff]">
            <thead>
                  ...
                  <tr key={bot._id} className="border-b">
                    ...
                    <td className="px-4 py-4 relative">
                      ...
                      {dropdownOpen === bot._id && (
                        <div
                          ref={dropdownRef}
                          className="absolute right-10 mt-2 z-[9] w-56 bg-white  shadow-lg rounded-lg py-2 "
                        >
                          <button
                            onClick={() => handleBotClick(bot._id)}
                  ...
                  <div className="flex flex-col md:flex-row gap-6">
                    ...
                    <div className="mt-8 pl-6 border-l">
                      ...
                      <div className="grid grid-cols-2 gap-2 sm:grid-cols-1 text-sm">
                        ...
                        <div className="grid grid-cols-4 col-span-4 gap-2 sm:grid-cols-1 pr-2 pl-2">
                        ...
                        <div className="w-max  pr-2 pl-2 mt-2 ">
                          <label className="block font-medium">
                            Chatbot colour
                          </label>
                          <div className="flex items-center border p-2 rounded-lg space-x-2 mt-1">
                            <div
                            ...
                            ></div>
                            <div className="flex items-center border border-gray-300 cursor-pointer rounded-md px-4  py-1 bg-gray-100 text-gray-700">
                              {config.color}
                              <input
                                type="color"
                                className=" cursor-pointer w-max  opacity-0 "
                                value={config.color}
                                onChange={(e) =>
                                  setConfig({
                                    ...config,
